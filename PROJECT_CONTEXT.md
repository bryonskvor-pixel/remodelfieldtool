...
- **Measurements:** numeric pad with unit presets per item; L×W screens compute SF live; running tallies visible (e.g., cumulative tile SF). **Room-shape sketch (added 2026-07-11, build next):** third pad mode for non-rectangular rooms — tap-to-place corners (not freehand; gloves), walls snap to horizontal/vertical, each wall's length entered on the numeric pad (auto-derivable closing segments are suggested for confirmation, never silently written — Hard Rule 1), computes floor SF/wall LF into the area row (overridable). Polygon stores as measurement JSON; L×W stays the default fast path.

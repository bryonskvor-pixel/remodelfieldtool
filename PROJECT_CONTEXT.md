...
**Intake: WIRED (2026-07-22).** The cleanconstructionllc.com contact form's serverless handler (`api/contact.js` in `bryonskvor-pixel/cleanconstruction`) now POSTs every inquiry to `POST /api/intake/:contractorId` (pilot contractor = Bryon's row) in addition to the Resend email — best-effort, so a ScopeWalk outage never blocks the email. Site form types map onto the ScopeWalk taxonomy (unmapped → `general`) with the original wording preserved in `intake_notes`. Leads land as `source=website_intake`, sync to the app via bootstrap, and surface in the Home "New leads" card. Verified end-to-end against production. When selling to other contractors, each gets their own intake URL (the endpoint is contractor-keyed already).
